...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect is insect.queen_ants[0]:
                return None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        #if no ant in bee's place, return False
        #if there is ant, but blocks_path = False
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        if self.container:
            return not self.ant and not other.container
        return False
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while count < self.min_range: #this will be skipped for shortthrower
            current_place = current_place.entrance
            count+=1
        while count <= self.max_range and current_place.entrance: #cycle from place min to place max
...
# BEGIN Problem 7
class WallAnt(Ant):
    """A WallAnt that does nothing. It has lots of armor. Hurray"""
    implemented = False
    name = 'Wall'
    food_cost = 4
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        QueenAnt.queen_ants.append(self) #adding a new queen to the list of queens
    ...
    def action(self, colony):
        ...
        if self is self.queen_ants[0]:
            ScubaThrower.action(self, colony)
            while current_exit:
                if current_exit.ant and current_exit.ant not in self.boosted_ants:
                    current_exit.ant.damage = current_exit.ant.damage * 2
                    if isinstance(current_exit.ant, BodyguardAnt) and current_exit.ant.ant and current_exit.ant.ant not in self.boosted_ants:
                        current_exit.ant.ant.damage = current_exit.ant.ant.damage * 2
                        self.boosted_ants.append(current_exit.ant.ant)
                    self.boosted_ants.append(current_exit.ant)
                elif current_exit.ant in self.boosted_ants and isinstance(current_exit.ant, BodyguardAnt) and current_exit.ant.ant not in self.boosted_ants:
                    current_exit.ant.ant.damage = current_exit.ant.ant.damage * 2
                    self.boosted_ants.append(current_exit.ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        pain = self.armor - amount
        if not pain and self is self.queen_ants[0]:
            bees_win()
        else:
